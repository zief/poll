package zief.romi.poll.service;
...
import zief.romi.poll.exception.BadRequestException;
import zief.romi.poll.exception.ResourceNotFoundException;
import zief.romi.poll.model.Choice;
import zief.romi.poll.model.ChoiceVoteCount;
import zief.romi.poll.model.Poll;
import zief.romi.poll.model.User;
import zief.romi.poll.model.Vote;
...
import zief.romi.poll.util.ModelMapper;
...
public class PollService {
  ...
  public Poll createPoll(PollRequest pollRequest) {
    ...
    Instant expirationDateTime =
        now.plus(Duration.ofDays(pollRequest.getPollLength().getDays()))
  ...
  public PollResponse castVoteAndGetUpdatedPoll(
      Long pollId, VoteRequest voteRequest, UserPrincipal currentUser) {
    Poll poll =
        pollRepository
            .findById(pollId)
            .orElseThrow(() -> new ResourceNotFoundException("Poll", "id", pollId));
    ...
    return ModelMapper.mapPollToPollResponse(
        poll, choiceVotesMap, creator, vote.getChoice().getId());
  }
    ...
    if (currentUser != null) {
      ...
      pollUserVoteMap =
          userVotes
              ...
              .collect(
                  Collectors.toMap(
                      vote -> vote.getPoll().getId(), vote -> vote.getChoice().getId()));
    }
  ...
  Map<Long, User> getPollCreatorMap(List<Poll> polls) {
    ...
    List<Long> creatorIds =
        polls.stream().map(Poll::getCreatedBy).distinct().collect(Collectors.toList());
